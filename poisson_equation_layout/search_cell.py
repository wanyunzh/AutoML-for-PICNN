...
class Noded(nn.Module):
    ...
    def forward(self, prev_nodes):
        ...
        if out[1].size() !=out[0].size():
            ...
            _, _, height2, width2 = out[1].size()
            if height1 > height2 or width1 > width2:
                out[1] = interpolate(out[1], (height1, width1))
            else:
                out[0] = interpolate(out[0], (height2, width2))
...
class CNN(nn.Module):
    ...
    def __init__(self, in_channels=1, channels=16, n_classes=1, n_layers=4, n_nodes=3,
                 stem_multiplier=4):
        super().__init__()
...
if __name__ == "__main__":
    ...
    exp_config.experiment_name = 'heat equation layout cell'
    ...
    import json
    json.dump(exported_arch_best, open('layout_cell.json', 'w'))
    with fixed_arch('layout_cell.json'):
