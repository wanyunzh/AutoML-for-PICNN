...
def plotMesh(ax,x,y,width=0.05):
	[ny,nx]=x.shape#先画y方向上的线
	for j in range(0,nx):
		ax.plot(x[:,j],y[:,j],color=cinternal,linewidth=width)
	for i in range(0,ny): #再画x方向上的线
...
def visualize2D(ax,x,y,u,colorbarPosition='vertical',colorlimit=None):
	xdg0=np.vstack([x.flatten(order='C'),y.flatten(order='C')]) #x是19*84格子各自的横坐标，y是纵坐标
...
class hcubeMesh(object):
	...
	def __init__(self,leftX,leftY,rightX,rightY,lowX,lowY,upX,upY,
		...
		# 	plt.show()
		# plt.close(fig)
		self.dxdxi=(self.x[1:-1,2:]-self.x[1:-1,0:-2])/2/self.h #partial x/partial ksai
		self.dydxi=(self.y[1:-1,2:]-self.y[1:-1,0:-2])/2/self.h
		self.dxdeta=(self.x[2:,1:-1]-self.x[0:-2,1:-1])/2/self.h #partial x/partial eta
