...
class CNN(nn.Module):
    ...
    def __init__(self, in_channels=1, channels=16, n_classes=1, n_layers=4, n_nodes=4,
                 stem_multiplier=4):
        super().__init__()
...
if __name__ == "__main__":
    ...
    exp_config.experiment_name = 'possion cell based'
    exp_config.trial_concurrency = 1 # 最多同时运行 2 个试验
    exp_config.max_trial_number = 20
    # exp_config.trial_gpu_number = 1
    # exp_config.training_service.use_active_gpu = True
    exp.run(exp_config, 8071)
    for model_dict in exp.export_top_models(top_k =3,formatter='dict'):
        print(model_dict)
    from nni.retiarii import fixed_arch
    exported_arch_best = exp.export_top_models(top_k =3,formatter='dict')[0]
    import json
    json.dump(exported_arch_best, open('possion_model_cell.json', 'w'))
    with fixed_arch('possion_model_cell.json'):
        final_model = CNN(n_layers=3)
