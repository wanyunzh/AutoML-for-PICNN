...
from evaluate_struct import traintest_burgers
...
if __name__ == "__main__":
    ...
    #     partial(traintest_burgers, step=steps, effective_step=effective_step)
    # )
    evaluator = FunctionalEvaluator(traintest_burgers)
    exp = RetiariiExperiment(model_space, evaluator, [], strategy.PolicyBasedRL(max_collect=3,trial_per_collect =1))
    exp_config = RetiariiExeConfig('local')
    exp_config.experiment_name = 'burgersequation'
    exp_config.trial_concurrency = 1 # 最多同时运行 2 个试验
    exp_config.max_trial_number = 3
    ...
    import json
    json.dump(exported_arch_best, open('burgers.json', 'w'))
