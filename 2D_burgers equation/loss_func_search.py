...
class PhyCRNet(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, 
        ...
        for i in range(self.num_encoder, self.num_encoder + self.num_convlstm):
            ...
            cell = ConvLSTMCell(
                input_channels = self.input_channels[i+1], #256
                hidden_channels = self.hidden_channels[i], #256
                input_kernel_size = self.input_kernel_size[i], #3
                input_stride = self.input_stride[i], #1
                input_padding = self.input_padding[i]) #1
            setattr(self, name, cell)
            self._all_layers.append(cell)  
        factors=1
...
# def compute_loss(output, loss_func,params,epoch,iteration):
...
def train(model, input, initial_state, n_iters, time_batch_size, learning_rate, 
          dt, dx, save_path, pre_model_save_path, num_time_batch,params):
...
if __name__ == '__main__':
    ...
    model_save_path = '../model/loss_search_self.pt'
    ...
    train_loss = train(model, input, initial_state, n_iters_adam, time_batch_size, 
        lr_adam, dt, dx, model_save_path, pre_model_save_path, num_time_batch,params)
    end = time.time()
    ...
    model = PhyCRNet(
        input_channels = 2, 
        hidden_channels = [32, 64, 128, 256],  #[32, 64, 128, 256,256]
        input_kernel_size = [4, 4, 4, 3], 
        input_stride = [2, 2, 2, 1], 
        input_padding = [1, 1, 1, 1],  
        dt = dt,
    ...
    np.save('losstruthall_search.npy',truthall)
    np.save('losspicnnall_search.npy',picnnall)
    ...
    np.savetxt('lossEV_self.txt', Error)
    np.savetxt('lossRes_list_self.txt', Res_list)
