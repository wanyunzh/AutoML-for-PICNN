...
class Pool2d_conv(nn.Module):
    ...
    def __init__(self, channels_p, c_cur,pool_type, kernel_size, stride):
        ...
        if pool_type.lower() == 'max':
            ...
            raise ValueError()
        layers = [
            nn.Conv2d(channels_p, c_cur, kernel_size=3, padding=1, padding_mode='circular'),
...
class StdConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(C_in, C_out, kernel_size, stride, padding, bias=False, padding_mode='circular'),
...
class SepConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding):
        ...
        group = 32 if C_out % 32 == 0 else 16
        self.net = nn.Sequential(
            nn.Conv2d(C_in, C_in, kernel_size, stride, padding, groups=C_in, padding_mode='circular'),
